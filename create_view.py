...
from src.config import MODEL_PATH_V2_IN_CONTAINER, get_clickhouse_connection_params
from src.inference import ensure_anomalous_events_view_v2


def main() -> int:
    try:
        client = Client(**get_clickhouse_connection_params())
        ensure_anomalous_events_view_v2(client, MODEL_PATH_V2_IN_CONTAINER)
    except NetworkError:
        ...
        return 1
    print(f"View anomalous_events_v2 created (model path: {MODEL_PATH_V2_IN_CONTAINER})")
    return 0
