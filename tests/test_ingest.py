"""Integration tests for ingest: row count and anomaly stats after run_ingest."""
...
def test_ingest_inserts_expected_row_count(ch_client: Client) -> None:
    """After run_ingest(n=1000), table has 1000 rows."""
    ensure_service_logs_table(ch_client)
    ch_client.execute(f"TRUNCATE TABLE IF EXISTS {SERVICE_LOGS_TABLE}")
    inserted = run_ingest(n=1_000, seed=99, client=ch_client)
    assert inserted == 1_000
    (count,) = ch_client.execute(f"SELECT count() FROM {SERVICE_LOGS_TABLE}")[0]
    assert count == 1_000
...
def test_ingest_anomaly_ratio_in_stored_data(ch_client: Client) -> None:
    ...
    ensure_service_logs_table(ch_client)
    ch_client.execute(f"TRUNCATE TABLE IF EXISTS {SERVICE_LOGS_TABLE}")
    run_ingest(n=5_000, seed=123, client=ch_client)
    result = ch_client.execute(
        f"SELECT countIf(is_anomaly = 1) AS anomalies, count() AS total FROM {SERVICE_LOGS_TABLE}"
    )
